   	...
   	if(flag=="up"){
	  	for(var j = 0; j <bubbleList.length;j++){
	   	  	var bheight1 = Number(bubbleList[j].y),
	   	  		/*bheight2 = Number(bubbleList[j+1].y)*/
	   	  	...
	   	  	if(newY > bheight1&& newY < bheight1+2){
	   	  		...
	   	  		if($("."+j).length > 0){
	   	  		   $("."+j).show();
	   	  			...
	   	  			$("<div class='bubble-tooltip "+j+"'><span>"+bubbleList[j].value+"</span></div>")
	   	  			.appendTo($("#main"))
	   	  			.css({
	   	  				"top":bheight1+"rem",
	   	  				"left":bleft+"rem"
   		...
   		for(var k = bubbleList.length -1; k >= 0;k--){
	   	  	...
	   	  	if(newY < bheight1-2.5){
	   	  		$("."+k).hide();  
	   	  		
	   	  	}else if(newY < bheight1 && newY > bheight1-1){
